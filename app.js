...
const dbPassword =
  process.env.DBPASSWORD || console.log("DBPASSWORD ontbreekt") || exit(1);
...
app.get("/", (req, res) => {
  const query =
    "SELECT Name, SigningDate FROM Entries ORDER BY SigningDate DESC";
...
app.listen(port, () => {
  ...
  const createTableQuery =
    "CREATE TABLE IF NOT EXISTS Entries (Id INT AUTO_INCREMENT PRIMARY KEY, Name VARCHAR(255) NOT NULL, SigningDate DATETIME NOT NULL)";
  connection.query(createDBQuery, (err) => {
    if (err) throw err;
  });
  connection.query("USE GuestBook", (err) => {
    if (err) throw err;
  });
